...
use soroban_sdk::auth::Context::Contract;
...
fn test<ChatMessage>() {
    ...
    let contract_address = env.register(super.Contract, ());
    ...
    env.as_contract(&contract_address, || {
        ...
        assert_eq!(env.storage().temporary().get::<_, ChatMessage>(&Storage::Chat(0)).unwrap().user, msg0);
    });

    client.send(&author1, &msg1);
    env.as_contract(&contract_address, || {
        assert_eq!(env.storage().instance().get::<_, u32>(&INDEX).unwrap(), 2);
        assert_eq!(env.storage().temporary().get::<_, ChatMessage>(&Storage::Chat(1)).unwrap().user, msg1);
    });
    client.send(&author2, &msg2);
    env.as_contract(&contract_address, || {
        assert_eq!(env.storage().instance().get::<_, u32>(&INDEX).unwrap(), 3);
        assert_eq!(env.storage().temporary().get::<_, ChatMessage>(&Storage::Chat(2)).unwrap().user, msg2);
    });
}